                            ...
                            IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath OPTIONAL)
{
    ...
    DEBUG((DEBUG_VERBOSE, "%a: Controller=%p\n", __FUNCTION__, ControllerHandle));
    ...
    if (!EFI_ERROR(rc))
    {
        ...
        DEBUG((DEBUG_VERBOSE, "%a: PciIoProtocol -> %r\n", __FUNCTION__, rc));
    return rcRet;
}
...
VBoxVgaMiniPortVMP_SetMode(IN EFI_VGA_MINI_PORT_PROTOCOL *This, IN UINTN ModeNumber)
{
     ...
     */
    r[63] = IoRead8((UINTN)0x3da);
    IoWrite8(0x3c0, 0);
    IoWrite16(0x3d4, 0x0011);
...
#define BOUTB(count, aport, dport)  \
     ...
     */
     ...
     r[63] = IoRead8(0x3da);            // reset flip/flop
     ...
     IoWrite8(0x3c0, 0x20);             // re-enable palette

     /* set all VBox extended registers */
     r[0] = 1;
     BOUTB(1, 0x3c4, 0x3c5);            // disable sequencer

     IoWrite16(0x1ce, 0x04); IoWrite16(0x1cf, 0);  // ENABLE
     ...
     /* Load default values into the first 16 entries of the DAC */
     {
         static const UINT8 s_a3bVgaDac[64*3] =
         {
          ...
          for (i = 0; i < 64; ++i)
          {
              IoWrite8(0x3c8, (UINT8)i);
              IoWrite8(0x3c9, s_a3bVgaDac[i*3 + 0]);
              IoWrite8(0x3c9, s_a3bVgaDac[i*3 + 1]);
              IoWrite8(0x3c9, s_a3bVgaDac[i*3 + 2]);
          }
     }
     ...
     /* Load the appropriate font into the first map */
     {
        ...
        // Enable font map access
        {
            ...
            IoWrite16(0x3c4, 0x0100);
            IoWrite16(0x3c4, 0x0402);
            IoWrite16(0x3c4, 0x0704);
            IoWrite16(0x3c4, 0x0300);
            /* Write graphics controller registers */
            IoWrite16(0x3ce, 0x0204);
            IoWrite16(0x3ce, 0x0005);
            IoWrite16(0x3ce, 0x0406);
        }
        ...
        IoWrite16(0x3d4, 0x4009 | ((height - 1) << 8));

        // Disable font map access again
        {
            /* Write sequencer registers */
            IoWrite16(0x3c4, 0x0100);
            IoWrite16(0x3c4, 0x0302);
            IoWrite16(0x3c4, 0x0304);
            IoWrite16(0x3c4, 0x0300);
            /* Write graphics controller registers */
            IoWrite16(0x3ce, 0x0004);
            IoWrite16(0x3ce, 0x1005);
            IoWrite16(0x3ce, 0x0e06);
        }
    }
