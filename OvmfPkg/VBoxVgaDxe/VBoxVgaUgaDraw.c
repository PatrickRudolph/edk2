  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "%a:%d VIDEO: %dx%d %d bpp\n", __FILE__, __LINE__,
         HorizontalResolution, VerticalResolution, ColorDepth));
  Private = VBOX_VGA_PRIVATE_DATA_FROM_UGA_DRAW_THIS (This);
  ...
  )
{
  ...
  //
  VBoxGetVmVariable (EFI_INFO_INDEX_HORIZONTAL_RESOLUTION,
                  (CHAR8 *)&HorizontalResolution,
                  sizeof(HorizontalResolution)
                  );
  VBoxGetVmVariable (EFI_INFO_INDEX_VERTICAL_RESOLUTION,
                  (CHAR8 *)&VerticalResolution,
  ...
  if (Index >= Private->MaxMode) {
    VBoxGetVmVariable(EFI_INFO_INDEX_GRAPHICS_MODE, (CHAR8 *)&Index, sizeof(Index));
