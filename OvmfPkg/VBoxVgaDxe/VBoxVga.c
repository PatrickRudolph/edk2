...
#define BOUTB(storage, count, aport, dport)                                   \
    do {                                                                      \
        for (i = 0 ; i < (count); ++i)                                        \
            if ((dport) == (aport) + 1)                                       \
                VBoxIoWrite16((aport), ((UINT16)storage[i] << 8) | (UINT8)i); \
            else {                                                            \
                IoWrite8((aport), (UINT8)i);                                  \
                IoWrite8((dport), storage[i]);                                \
            }                                                                 \
    } while (0)
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_VERBOSE, "%a:%d status:%r\n", __FILE__, __LINE__, Status));
    return Status;
  }
  ...
  )
{
  ...
  //
  Private->VRAMSize = VBoxIoRead32(VBE_DISPI_IOPORT_DATA);
...
{
  IoWrite8(PALETTE_INDEX_REGISTER, (UINT8) Index);
  IoWrite8(PALETTE_DATA_REGISTER, (UINT8) (Red >> 2));
  IoWrite8(PALETTE_DATA_REGISTER, (UINT8) (Green >> 2));
  IoWrite8(PALETTE_DATA_REGISTER, (UINT8) (Blue >> 2));
}
  ...
  for (RedIndex = 0; RedIndex < 8; RedIndex++) {
    for (GreenIndex = 0; GreenIndex < 8; GreenIndex++) {
      for (BlueIndex = 0; BlueIndex < 4; BlueIndex++) {
        SetPaletteColor (Private, Index, (UINT8) (RedIndex << 5),
                         (UINT8) (GreenIndex << 5), (UINT8) (BlueIndex << 6));
        Index++;
      }
    }
  }
#else
     {
         ...
         static const UINT8 s_a3bVgaDac[64*3] =
         {
          ...
          for (i = 0; i < 64; ++i)
          {
              IoWrite8(PALETTE_INDEX_REGISTER, (UINT8)i);
              IoWrite8(PALETTE_DATA_REGISTER, s_a3bVgaDac[i*3 + 0]);
              IoWrite8(PALETTE_DATA_REGISTER, s_a3bVgaDac[i*3 + 1]);
              IoWrite8(PALETTE_DATA_REGISTER, s_a3bVgaDac[i*3 + 2]);
          }
     }
  ...
  )
{
    ...
    DEBUG ((DEBUG_INFO, "%a:%d InitializeGraphicsMode: %dx%d bpp:%d\n",
            __FILE__, __LINE__, ModeData->Width, ModeData->Height,
            ModeData->ColorDepth));
    ...
    IoWrite8(MISC_OUTPUT_REGISTER, 0xc3);
    VBoxIoWrite16(SEQ_ADDRESS_REGISTER, 0x0204);

    IoRead8(INPUT_STATUS_1_REGISTER);   // reset attribute address/data flip-flop
    IoWrite8(ATT_ADDRESS_REGISTER, 0);  // blank screen using the attribute address register

    VBoxIoWrite16(CRTC_ADDRESS_REGISTER, 0x0011);

    VBoxIoWrite16(SEQ_ADDRESS_REGISTER, 0x0100);
    ...
    VBoxIoWrite16(SEQ_ADDRESS_REGISTER, 0x0300);
    ...
    IoWrite8(MISC_OUTPUT_REGISTER, ModeData->MiscSetting);

    if (ModeData->ColorDepth <= 8) {
      IoWrite8(DAC_PIXEL_MASK_REGISTER, 0xff);
      SetDefaultPalette(Private);
    }

    if (!ModeData->CrtcSettings) {
        // No CRTC settings, use VBE
        VBoxIoWrite16(VBE_DISPI_IOPORT_INDEX, 0x00);
        VBoxIoWrite16(VBE_DISPI_IOPORT_DATA, 0xb0c0);                       // ID
        VBoxIoWrite16(VBE_DISPI_IOPORT_INDEX, 0x04);
        VBoxIoWrite16(VBE_DISPI_IOPORT_DATA, 0);                            // ENABLE
        VBoxIoWrite16(VBE_DISPI_IOPORT_INDEX, 0x01);
        VBoxIoWrite16(VBE_DISPI_IOPORT_DATA, (UINT16)ModeData->Width);      // XRES
        VBoxIoWrite16(VBE_DISPI_IOPORT_INDEX, 0x02);
        VBoxIoWrite16(VBE_DISPI_IOPORT_DATA, (UINT16)ModeData->Height);     // YRES
        VBoxIoWrite16(VBE_DISPI_IOPORT_INDEX, 0x03);
        VBoxIoWrite16(VBE_DISPI_IOPORT_DATA, (UINT16)ModeData->ColorDepth); // BPP
        VBoxIoWrite16(VBE_DISPI_IOPORT_INDEX, 0x05);
        VBoxIoWrite16(VBE_DISPI_IOPORT_DATA, 0);                            // BANK
        VBoxIoWrite16(VBE_DISPI_IOPORT_INDEX, 0x06);
        VBoxIoWrite16(VBE_DISPI_IOPORT_DATA, (UINT16)ModeData->Width);      // VIRT_WIDTH
        VBoxIoWrite16(VBE_DISPI_IOPORT_INDEX, 0x07);
        VBoxIoWrite16(VBE_DISPI_IOPORT_DATA, (UINT16)ModeData->Height);     // VIRT_HEIGHT
        VBoxIoWrite16(VBE_DISPI_IOPORT_INDEX, 0x08);
        VBoxIoWrite16(VBE_DISPI_IOPORT_DATA, 0);                            // X_OFFSET
        VBoxIoWrite16(VBE_DISPI_IOPORT_INDEX, 0x09);
        VBoxIoWrite16(VBE_DISPI_IOPORT_DATA, 0);                            // Y_OFFSET
        VBoxIoWrite16(VBE_DISPI_IOPORT_INDEX, 0x04);
        VBoxIoWrite16(VBE_DISPI_IOPORT_DATA, 1);                            // ENABLE
        /// @todo enabling VBE is automatically tweaking the CRTC, GC, SC, clears
        // the screen and at the end unblanks graphics. So make sure that
        // nothing is done after this which needs blanking. Way too much magic,
    ...
    IoRead8(INPUT_STATUS_1_REGISTER);       // reset attribute address/data flip-flop
    IoWrite8(ATT_ADDRESS_REGISTER, 0x20);   // unblank screen
